...
st.set_page_config(page_title="Projet Dashboard Vélib", page_icon="🚲", layout="wide")
    ...
    hist = (
        ...
        .encode(
            x=alt.X(
                "velos_disponibles:Q",
                bin=alt.Bin(step=1),
                axis=alt.Axis(title="Nombre de vélos disponibles", format="d"),
            ),
            y=alt.Y("count():Q", title="Nombre de stations"),
            tooltip=[alt.Tooltip("count():Q", title="Stations")],
        )
        .properties(height=400)
    )
